...
namespace CrazyDriver.View
{
    ...
    public sealed class CameraRig : MonoBehaviour
    {
        ...
        [Inject]
        public void Construct(GameConstantsSO constants) => _settings = constants.Camera;
        ...
        // Last thing in the frame: it reads the transforms every other view has just written.
        private void LateUpdate()
        {
            if (_settings == null || _motor == null)
            {
                return;
            }
            ...
            GetTargets(_motor, out Vector3 targetPosition, out Quaternion targetRotation);

            Vector3 position = Vector3.SmoothDamp(
                transform.position, targetPosition, ref _positionVelocity,
                _settings.PositionSmoothingTime, Mathf.Infinity, deltaTime);
            ...
            transform.SetPositionAndRotation(
                position, Quaternion.Slerp(transform.rotation, targetRotation, rotationBlend));
        }

        private void GetTargets(CarMotor car, out Vector3 position, out Quaternion rotation)
        {
